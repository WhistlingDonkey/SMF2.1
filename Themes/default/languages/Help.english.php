...
// All the HTTP Security settings
$helptxt['securitysettings'] = '<strong>Security Settings</strong><br>
Here you can set security settings for your forum. It is strongly recommend to understand the settings before applying them as can you can render you forum inaccessable with a bad configuration.';
$helptxt['httpsec_x-frame-options'] = 'This response header can be used to indicate whether or not a browser should be allowed to render a page in a &lt;frame&gt;, &lt;iframe&gt;, &lt;embed&gt; or &lt;object&gt;. Sites can use this to avoid click-jacking attacks by ensuring that their content is not embedded into other sites. This setting is depreciated by using the <em>frame-ancestors</em> directive in a Content Security Policy';
$helptxt['httpsec_x-content-type-options'] = 'This respose header can be used to instruct the browser to not try and guess mime types for files by inspecting them but to strictly use the mime type header that is sent with a resource';
$helptxt['httpsec_strict-transport-security'] = 'This response header instructs browsers that the site should only be accessed using HTTPS instead of using HTTP. <em>Include Subdomains</em> includes the sites subdomains in the policy. <em>Preload</em> is non standard response header and should be considered experimental. You should thoroughly understand preload for HTTP Strict Transport Security before enabling it.<br><em>max-age</em> is the time in seconds that the browser should remember that a site is only to be accessed using HTTPS. The max-age must be at least 31536000 seconds (1 year) if using Preload';
$helptxt['httpsec_cross-origin-embedder-policy'] = '<em>Todo:</em> This response header prevents a document from loading any cross-origin resources that don\'t explicitly grant the document permission';
$helptxt['httpsec_cross-origin-opener-policy'] = '<em>Todo:</em> This response header allows you to ensure a top-level document does not share a browsing context group with cross-origin documents.';
$helptxt['httpsec_cross-origin-resource-policy'] = '<em>Todo:</em> This response header conveys a desire that the browser blocks no-cors cross-origin/cross-site requests to the given resource.';
$helptxt['httpsec_content-security-policy'] = 'This response header allows you to control \'what and where from\' resources are allowed to load and run on your website in the users browser. This helps prevent many types of attacks if implemented correctly.<br><br> Do <strong>not</strong> set a Content Security Policy here if you are setting a Content Security Policy for this website with your server. <br><br><strong>Important:</strong> Make sure you absolutely know what you are doing with what you put into the Content Security Policy, it is entirely possible to lock yourself out of your own website by preventing critical files to load.';
$helptxt['httpsec_referrer-policy'] = 'This response header controls how much referrer information is included with requests. The best privacy setting is no-referrer';
$helptxt['httpsec_clear-site-data'] = 'The Clear-Site-Data header can be sent when a user logs out of the forum.  Valid values, including the quotation marks  are "cookies", "cache", "storage" and "execution" or "*" for all values. Each value must be seperated by a comma.';

$helptxt['httpsec_nonce'] = 'A nonce can be provided as a one time token for the scripts and styles used by the forum. This provides protection against attacks from injected scripts and styles. <em>Disable</em> disables the use of nonces, <em>Forum generated</em> uses the forum to generate the nonces and <em>Sever generated</em> sets the forum to use a nonce generated by the server if your server is set up to generate nonces. You should only use server generated nonces if your server can output a base64 encoded nonce.<br><br>The nonce is set in the Content Security Policy as: script-src \'nonce-&lt;noncepattern&gt;\' and style-src \'nonce-&lt;noncepattern&gt;\'where &lt;noncepattern&gt; is the pattern you set.';
$helptxt['httpsec_csp_nonce_pattern'] = 'This is the pattern that will be replaced by the actual nonce in the Content Security Policy when the header is sent, eg %nonce%.';
$helptxt['httpsec_server_nonce_param'] = 'This is the parameter passed by the server to use as the nonce.';
$helptxt['httpsec_nonce_applies'] = 'Specifies whether the nonce will apply to inline scripts and styles only (CSP 2.0) or to all scripts and styles (CSP 3.0).';

$helptxt['httpsec_custom_headers'] = 'List of custom response headers to be sent in the HTTP response. The list is one header per line.';
