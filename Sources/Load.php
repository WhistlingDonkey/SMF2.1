...
function template_include($filename, $once = false)
{
	...
	if ($file_found !== true)
	{
		...
		if (!isset($txt['template_parse_error']))
		{
			...
			$txt['template_parse_error_message'] = 'It seems something has gone sour on the forum with the template system.  This problem should only be temporary, so please come back later and try again.  If you continue to see this message, please contact the administrator.<br><br>You can also try <button type="button" class="link-btn" onclick="javascript:location.reload();">refreshing this page</button>.';
			$txt['template_parse_error_details'] = 'There was a problem loading the <pre><strong>%1$s</strong></pre> template or language file.  Please check the syntax and try again - remember, single quotes (<pre>\'</pre>) often have to be escaped with a slash (<pre>\\</pre>).  To see more specific error information from PHP, try <a href="%2$s%1$s" class="extern">accessing the file directly</a>.<br><br>You may want to try to <button type="button" class="link-btn" onclick="javascript:location.reload();">refresh this page</button> or <a href="%3$s?theme=1">use the default theme</a>.';
